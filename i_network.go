...
// NewNetwork returns an INetwork object based on an IDispatch object.
func NewNetwork(idispatch *ole.IDispatch) INetwork {
...
func (n *iNetwork) SetCategory(cat NLMNetworkCategory) (err error) {
	byteCategory := byte(cat)
	hr, _, _ := globalSyscaller.SyscallN(
		(*iNetworkVTable)(unsafe.Pointer(n.idispatch.RawVTable)).SetCategory,
		uintptr(unsafe.Pointer(n.idispatch)),
		uintptr(unsafe.Pointer(&byteCategory)),
	)
	if hr < 0 {
