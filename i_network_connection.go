...
func NewNetworkConnectionFromVariant(variant *ole.VARIANT) (INetworkConnection, error) {
	networkConnectionDispatch, err := asINetworkConnectionIDispatch(variant.ToIUnknown())
	if err != nil {
		return nil, fmt.Errorf("failed to convert variant to NetworkConnection interface: %v", err)
	}
	return &iNetworkConnection{idispatch: networkConnectionDispatch}, nil
}
...
func (nc *iNetworkConnection) GetNetwork() (INetwork, error) {
	...
	return NewNetwork(&idispatch), nil
}
...
func asINetworkConnectionIDispatch(unknown *ole.IUnknown) (*ole.IDispatch, error) {
	if unknown == nil {
	...
	dispatch, err := unknown.QueryInterface(interfaceGUID)
	if err != nil {
